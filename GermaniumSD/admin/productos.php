...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO productos (nombre, descripcion, precio_normal, precio_rebajado, cantidad, imagen, id_categoria) VALUES (:nombre, :descripcion, :p_normal, :p_rebajado, :cantidad, :foto, :categoria)");
            $stmt->bindParam(':nombre', $nombre);
            $stmt->bindParam(':descripcion', $descripcion);
            $stmt->bindParam(':p_normal', $p_normal);
            $stmt->bindParam(':p_rebajado', $p_rebajado);
    ...
    <div class="col-md-12">
        <div class="table-responsive">
            <table class="table table-hover table-bordered" style="width: 100%;">
                ...
                <tbody>
                    <?php
                    $query = $pdo->query("SELECT p.*, c.id AS id_cat, c.categoria FROM productos p INNER JOIN categorias c ON c.id = p.id_categoria ORDER BY p.id DESC");
